% Dynamics for genetic algorithm parameters
...
% Generate an individual (assign each surgery to an operating room)
...
% Evaluate an individual (load balance between rooms)
evaluate(Ind, Score) :-
    findall(R, operating_room(R), RoomList),
    calculate_load(RoomList, Ind, LoadList),
    calculate_balance(LoadList, Score).

% Calculate the load of each room
calculate_load([], _, []).
...
generate_generation(N, G, Pop) :-
    ...
    N1 is N + 1,
    generate_generation(N1, G, NewPopOrd).
